...
const shootingeffect = new Audio('gunshotsound.mp3');
shootingeffect.preload = 'auto';  // Preload the sound to avoid delays
const hiteffect = new Audio('playerhitsound.mp3');
const metalhiteffect = new Audio('metalsound.mp3');
const winsound = new Audio('winsound.wav');
const bgm = new Audio('gamebgm.mp3')
...
function startGame() {
    window.location.href = "gameScreen.html"; // Replace with the actual file name
}

function goBack() {
    const clickeffect = new Audio('pop-sound-effect-197846.mp3');
    clickeffect.play();
...
function showInstructions() {
    const clickeffect = new Audio('pop-sound-effect-197846.mp3');
    clickeffect.play();
    window.location.href = "instructions-page.html"; // Replace with the actual file name for instructions
}
...
function drawText() {
    // Set text properties
    c.font = '48px cursive'; // Adjust size of the text
    ...
    // First text - between 156px and 0px
    c.translate(78, canvas.height / 2); // Move to the first position
    c.rotate(Math.PI / 2 * 3); // Rotate by 90 degrees (Math.PI / 2 radians)
    c.fillText('CAPTURE!', 0, 0); // Draw the first text
    c.restore(); // Restore the context to the default state
    
    // Save the current context again for the second text
    c.save();

    // Second text - between 922px and 1080px
    c.translate(1001, canvas.height / 2); // Move to the second position (near 1080px)
    c.rotate(Math.PI / 2); // Rotate by 90 degrees for vertical text
    c.fillText('CAPTURE!', 0, 0); // Draw the second text
    c.restore(); // Restore the context again to the default state
}
...
class Player {
    ...
    isColliding(newX, newY) {
        return rectangles.some(rect =>
            newX + this.radius >= rect.x +2 &&
            newX - this.radius <= rect.x + rect.width -2&&
            newY + this.radius >= rect.y +2&&
            newY - this.radius <= rect.y + rect.height -2
        );
    }
...
const rectangles = [
    { x: 293, y: 75, width: 221, height: 151 },
    { x: 365, y: 365, width: 96, height: 57 },
    { x: 666, y: 138, width: 96, height: 57 },
    { x: 576, y: 279, width: 221, height: 151 },
];

const player1 = new Player(223.5, 167, 'black');
const player2 = new Player(870, 396, 'grey');
let winner = null;
let winner2 = null;
...
function displayWinMessage() {
    ...
    c.fillText(`${winner} Wins!`, canvas.width / 2, 188); 

    const backtomenu = document.getElementById('backtomenu');
    backtomenu.style.display = 'block'; // Make the button visible
    backtomenu.style.left = `${canvas.width / 2 }px`; // Center horizontally
    backtomenu.style.top = `${canvas.height / 2 + 50}px`; // Position below the message
}
...
function displayWinMessage2() {
    ...
    c.textAlign = 'center';
    c.textBaseline = 'middle';
    c.fillText(`${winner2} captures the area!`, canvas.width / 2, 188);
